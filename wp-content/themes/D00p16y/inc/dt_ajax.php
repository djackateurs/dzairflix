    ...
    if ( $query->have_posts() ) {
        ...
        while ( $query->have_posts() ) {
			...
			if ( has_post_thumbnail() ) {
				...
				$data[$post->ID]['img']	= dt_image_search('dt_poster', $post->ID, 'w90', false, true );
			} else {
...
function update_imdb_rating() {
	$key = get_option("dbmovies_private_key");
	$id = $_POST['id'];
	$imdb = $_POST['imdb'];
	if( (current_user_can('administrator')) && ($key)){
...
/* Register dbmovies API data
-------------------------------------------------------------------------------
*/
function register_site_db() {
	if(current_user_can('administrator')) {
		$action		= $_POST['action'];
		$option		= get_option('dbm_register_api_key');
		$domain		= get_option("siteurl");
		$key		= get_option( DT_KEY_S );
		$private	= get_option("dbmovies_private_key");
		if($action = 'register') {
			$api	= wp_remote_get( dbmur. 'sites/?key='.$key .'&url='. dt_domain( $domain ) .'&private='. $private );
			$json	= wp_remote_retrieve_body($api);
			$meta	= json_decode($json, TRUE);
			$options = array(
				// optional ( only customers )
				'customer'	=> $meta['customer'],
				// Requireds
				'domain'	=> dt_domain( $domain ),
				'private'	=> $private,
				'action'	=> $meta['action'],
			);
			if($meta['action'] == 'progress') {
				update_option('dbm_register_api_key', $options);
				update_option('dt_register_dbapi', '1');
			}
		}
	}
	die();
}
add_action('wp_ajax_register_site_db', 'register_site_db');
add_action('wp_ajax_nopriv_register_site_db', 'register_site_db');

/* Register dbmovies APP
-------------------------------------------------------------------------------
*/
function register_site_app() {
	// Json header
...
add_action('wp_ajax_dbmapp', 'register_site_app');
add_action('wp_ajax_nopriv_dbmapp', 'register_site_app');

/* Register & refresh dbmovies API data
-------------------------------------------------------------------------------
*/
function dt_dbmv() {
	// Json header
	...
	$key		= get_option( DT_KEY_S );
	$time		= time();
	$action		= dbmupdate('action');
	if($id == $private) {
		// Option deleted for data update 
		if($del == 'true') { 
			delete_option('dbm_register_api_key'); 
			delete_option('dt_register_dbapi'); 
		} 
		$arr['sucess']	= 'true';
		$arr['action']	= $action;
		$arr['license'] = $key;
		$arr['email']	= $email;
		$arr['time']	= "$time";
		$arr['dbmovies']['url']		= dt_domain($domain);
		$arr['dbmovies']['private'] = $private;
		$arr['dbmovies']['private'] = $private;
		$arr['tmdb']['api_key']		= $tmdbkey;
		$arr['tmdb']['api_lang']	= $tmdblang;
		echo json_encode($arr);
	} else {
		$arr['error'] = 'invalid_key';
		echo json_encode($arr);
	}
	die();
}
add_action('wp_ajax_dbmv', 'dt_dbmv');
add_action('wp_ajax_nopriv_dbmv', 'dt_dbmv');